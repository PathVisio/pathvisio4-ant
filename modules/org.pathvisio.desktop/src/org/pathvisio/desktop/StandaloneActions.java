...
/**
 * collection of menu / toolbar Actions that are only needed in the standalone
 * (non-applet) version of PathVisio.
 */
public class StandaloneActions implements ApplicationEventListener {
	...
	/**
	 * Open the online help in a browser window. In menu->help->help or F1
	 */
	...
	/**
	 * Help -> Plugin Manager Show a list of active plugins and errors that occurred
	 * while initializing the plugin manager.
	 */
	...
	/**
	 * Plugins -> Install local plugins allows users to start all bundles in a
	 * directory
	 */
	...
	/**
	 * Open a pathway from disk. In menu->file->open
	 */
	...
	/**
	 * Create a new pathway action In menu->file->new pathway
	 */
	...
	/**
	 * Show preferences dialog. Invoked in menu->edit->preferences
	 */
	...
	public static class PrintAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			/*
			 * check whether the user canceled or okayed the Page Setup Dialogue by
			 * comparing thePageFormat objects if canceled, cancel the Print operation
			 */
			...
			pj.setPrintable(new Printable() {
				...
				public int print(java.awt.Graphics graphics, PageFormat pageFormat, int pageIndex)
					...
					// scaling ratio is being set to the minimum of the two, so that the scaling
					// covers even the larger one.
					double minScale = Math.min(xScale, yScale);
					g2.translate(pageFormat.getImageableX(), pageFormat.getImageableY());
					// scaling both x and y using the minimum of the 2 scale ratios calculated
					// above.
					// i.e using the same scaling ratio for both x and y axis to maintain the aspect
					// ratio (width:height)
			...
			try {
				...
				// may be popup a dialogue saying that there's a problem with the print system
				// (printer problems)
