...
public class LocalRepositoryHandler {
	...
	/**
	 * Initializes the local repository if pathvisio.xml exists it reads the file
	...
	private void setUpLocalRepo() {
		...
		for (BundleVersion version : localRepository.getBundleVersions()) {
			Logger.log.info("Install bundle from local repository (" + version.getSymbolicName() + ", "
					+ version.getVersion() + ")");
			try {
		...
		for (Bundle b : bundleList) {
			BundleVersion bundleVersion = localRepository.getBundle(b.getSymbolicName(),
	...
	/**
	 * updates local xml repository file when a new plugin is installed or a plugin
	 * is uninstalled
	 */
